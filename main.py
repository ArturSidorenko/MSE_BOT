...
def find_classes_by_surname(cursor, teacher_surname, selected_day,
                            selected_month):
  sql = '''
        SELECT classes.*
        FROM classes
    ...
    '''
  cursor.execute(
    sql, (teacher_surname, selected_month, selected_day, selected_month))
  # Fetch the results and store them in a list of tuples
  data = cursor.fetchall()
  return data


def find_classes_by_teacher_id(cursor, teacher_id, selected_day,
                               selected_month):
  sql = '''
        SELECT classes.*
        FROM classes
        ...
        AND ((classes.month = ? AND classes.day >= ? )
        OR classes.month > ?))
        ORDER BY classes.month, classes.day;
    '''
  cursor.execute(sql,
                 (teacher_id, selected_month, selected_day, selected_month))
  # Fetch the results and store them in a list of tuples
  data = cursor.fetchall()
  return data
...
def full_name_by_surname(cursor, surname):
  cursor.execute('''SELECT full_name FROM teachers WHERE last_name = ? ''',
                 (surname, ))
  data = cursor.fetchall()
  return data[0][0]
  ...
  def handle_find_command(message):
    try:
      ...
      bot.send_message(message.chat.id,
                       'Напишите фамилию преподавателя, которого надо найти:')
  ...
  def handle_surname(message):
    ...
    if number is not None:
      ...
      if len(locations) > 0:
        bot.reply_to(
          message,
          f"Преподавателя *{full_name}* можно встретить в следующие дни:\n{text_locations}",
          parse_mode='Markdown')
      else:
        bot.reply_to(
          message,
          f"К сожалению, не удалось найти, в какие дни можно встретить преподавателя *{full_name}*.",
          parse_mode='Markdown')
    else:
      ...
      if similar_surnames:
        ...
        bot.reply_to(
          message,
          f"{no_teacher_report} {suggestion_phrase}\n\t\t\t*{suggestions}*\n{suggestion_phrase2}",
          parse_mode='Markdown')
      else:
        bot.reply_to(message,
                     f"Введенная фамилия отсутствует в списке преподаватей.",
                     parse_mode='Markdown')
